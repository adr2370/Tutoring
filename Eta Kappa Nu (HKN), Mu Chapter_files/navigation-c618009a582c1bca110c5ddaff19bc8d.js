 ...
 */
$(document).ready(function(){$(".submenu").hide(),current=location.pathname.replace("/",""),current.indexOf("/")!=-1&&(current=current.substring(0,current.indexOf("/"))),id=current,id.length!=0&&id!="coursesurveys"&&$("#"+id).length!=0?($("#"+id).addClass("current"),$("."+current+"_submenu").show()):($($(".navigation_toplevel_item")[0]).addClass("current"),$("#submenu .submenu").first().show(),$("#submenu-thin .submenu").first().show()),$(".navigation_toplevel_item").click(function(e){e.preventDefault(),$(".navigation_toplevel_item").removeClass("current"),$(this).addClass("current"),$(this).blur(),$(".submenu").hide(),id=$(this).attr("id")+"_submenu",$("."+id).show()}),$("#userbar").toggle(function(){$("#user-dropdown").slideDown(300)},function(){$("#user-dropdown").slideUp(300)})});